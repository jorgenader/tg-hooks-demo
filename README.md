...
Browser support is defined in the `hooks_demo/browserslist` file that is used for autoprefixing CSS.
...
|run psql                              |`make psql`                            |`docker-compose exec postgres psql --user hooks_demo --dbname hooks_demo` |
|generate docs                         |`make docs`                            |`docker-compose run --rm django sphinx-build ./docs ./docs/_build`          |

## Running commands on the server

    docker-compose -f docker-compose.production.yml run --rm --name hooks_demo_tmp django python manage.py <command>
