...
${prefix}_DATABASE_NAME=hooks_demo
${prefix}_DATABASE_USER=hooks_demo
        ...
        "letsencrypt": [
            {
                ...
                "filename": "letsencrypt.hooks_demo.conf",
        ...
        "nginx": [
            {
                ...
                "filename": "app.hooks_demo.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "common.hooks_demo.node.include",
                "filename": "common.hooks_demo.node.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "common.hooks_demo.django.include",
                "filename": "common.hooks_demo.django.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "app.hooks_demo.proxy_django.include",
                "filename": "app.hooks_demo.proxy_django.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "app.hooks_demo.proxy_node.include",
                "filename": "app.hooks_demo.proxy_node.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "app.hooks_demo.proxy.include",
                "filename": "app.hooks_demo.proxy.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "pattern": "%(target)s.ssl",
                "filename": "ssl.hooks_demo.include",
                "remote_path": "/etc/nginx/conf.d/%(filename)s"
            },
            {
                "default_site": True,
...
def test():
    ...
    defaults()
    env.node_site = 'hooks-demo.test.TODO.com'
    env.django_site = 'office.hooks-demo.test.TODO.com'
...
def setup_server(id=None):
    ...
    # Create database
    sudo('echo "CREATE DATABASE hooks_demo; '
         '      CREATE USER hooks_demo WITH password \'{db_password}\'; '
         '      GRANT ALL PRIVILEGES ON DATABASE hooks_demo to hooks_demo;" '
    ...
    django_settings_file = env.code_dir + '/hooks_demo/django.env'
    ...
    # Create log dir
    sudo('mkdir -p /var/log/hooks_demo/')
    ...
    with cd(env.code_dir):
        sudo('cp deploy/logrotate.conf /etc/logrotate.d/hooks_demo')
...
def add_secret_keys(component=None):
    ...
    if component == 'node':
        ...
        remote_path = env.code_dir + '/hooks_demo/django.env'
...
def collectstatic():
    docker_compose_run('node', 'npm run export-assets', name='hooks_demo_npm_export')
...
def docker_compose_run(service, cmd='', name='hooks_demo_tmp'):
...
def ensure_docker_networks():
    ...
    ensure_docker_network_exists('hooks_demo_default', [], internal=False)
    ensure_docker_network_exists('hooks_demo_nginx', ['nginx'])
    ensure_docker_network_exists('hooks_demo_postgres', ['postgres-{postgres_version}'.format(
        postgres_version=env.postgres_version)])
